...
def build_support_dependencies(evidence_pack: dict) -> dict:
    ...
    for item in evidence_pack.get('artifacts', []):
        ...
        elif (any(p in {'code', 'configs'} for p in path.parts) and
...
按问题编号从小到大运行。先查看相应程序的 --help；数值计算入口一般接受
--config（同版本 configs 中配置）、--output-dir（新建独立输出目录）。
各入口实际参数以其 --help 为准。生成图片前先准备相应计算输出，不能把交付图片当作计算输入。
