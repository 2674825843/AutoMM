...
def lock() -> FileLock:
    ...
    return FileLock(
        ROOT / item.get("path", "runtime/locks/orchestrator.lock"), int(item.get("stale_after_minutes", 60)) * 60
    )
...
def _run_mail_poll() -> None:
    ...
    result = subprocess.run(
        [sys.executable, str(ROOT / "scripts" / "notify_email.py"), "poll"],
        cwd=ROOT,
        capture_output=True,
        text=True,
        encoding="utf-8",
...
def _notify(kind: str, problem_id: str, question_id: str | None = None) -> dict[str, Any]:
    ...
    command = [
        sys.executable,
        str(ROOT / "scripts" / "notify_email.py"),
        "send",
        "--kind",
        kind,
        "--message",
    ...
    result = subprocess.run(
        command, cwd=ROOT, capture_output=True, text=True, encoding="utf-8", errors="replace", shell=False, check=False
    )
...
def recover_incomplete_transactions(journal: Path) -> list[str]:
    ...
    incomplete = sorted(
        action_id
        for action_id, phase in latest.items()
        if phase not in {"committed", "failed", "recovered_incomplete", "retrying"}
    )
    for action_id in incomplete:
        append_jsonl(
            journal,
            {
                "at": utc_now(),
                "action_id": action_id,
                "phase": "recovered_incomplete",
                "policy": "reconcile_without_replay",
            },
        )
    return incomplete
...
def execute_non_agent(action: dict[str, Any]) -> dict[str, Any]:
    ...
    if name == "create_assumption_candidate":
        return {
            "handled": name,
            "path": relative(create_assumption_version(action["problem_id"], action["question_id"])),
        }
    if name == "create_formulation_candidate":
        return {
            "handled": name,
            "path": relative(create_formulation_version(action["problem_id"], action["question_id"])),
        }
    if name == "advance_question":
        return {
            "handled": name,
            "state": transition(
                target_stage="problem_understanding",
                problem_id=action["problem_id"],
                question_id=action["question_id"],
                reason=action["reason"],
            ),
    ...
    if name == "advance_global_review":
        return {
            "handled": name,
            "state": transition(
                target_stage="cross_question_review", problem_id=action["problem_id"], reason=action["reason"]
            ),
        }
    ...
    if name == "send_problem_notification":
        ...
        problem["completion_notification"] = (
            result["status"]
            if result["status"] in {"sent", "skipped_disabled"}
            else "failed_final"
            if attempts >= 3
            else "failed"
        )
...
def run_once() -> dict[str, Any]:
    ...
    try:
        ...
        if recovered:
            append_jsonl(
                RUNTIME_DIR / "recovery.jsonl",
                {
                    "at": utc_now(),
                    "actions": recovered,
                    "task_changes": [item.get("task_id") for item in task_recovery],
                },
            )
        ...
        if action["action"] in {"run_agent", "inspect_compute_result", "route_compute_failure"}:
            try:
                ...
                note_recovery(
                    ...
                    error=str(exc),
                    progress={"productive": False, "stage": action.get("stage")},
                ...
                save_state(
                    state, event="agent_timeout_retry", details={"action_id": action_id, "stage": action.get("stage")}
                )
                append_jsonl(
                    journal,
                    {
        ...
        if agent_response and agent_response["status"] in {"failed", "blocked"}:
            reasons = agent_response.get("blocking_reasons") or [
                f"Agent {action['agent']} 返回 {agent_response['status']}"
            ]
            failure_class = str(agent_response.get("failure_class") or "code_runtime")
            note_recovery(
                state,
                failure_class=failure_class,
                error="; ".join(str(item) for item in reasons),
                progress={"productive": False, "stage": action.get("stage")},
                problem_id=action.get("problem_id"),
                question_id=action.get("question_id"),
            )
            state["blocking"] = (
                reasons if state.get("recovery_status") in {"human_blocked", "harness_invariant_error"} else []
            )
        ...
        note_recovery(
            ...
            failure_class=failure_class,
            error=str(exc),
            progress={"productive": False, "stage": action.get("stage") if "action" in locals() else None},
            problem_id=action.get("problem_id") if "action" in locals() else None,
            question_id=action.get("question_id") if "action" in locals() else None,
        )
        state["blocking"] = (
            [f"Runner {action_id} 失败：{exc}"]
            if failure_class in {"harness_invariant", "external_human_required"}
            else []
        )
        ...
        save_state(
            state,
            event="runner_action_recovery",
            details={"action_id": action_id, "error": str(exc), "failure_class": failure_class},
        )
        append_jsonl(
            journal,
            {
                "at": utc_now(),
                "action_id": action_id,
                "phase": state.get("recovery_status", "failed"),
                "failure_class": failure_class,
                "error": str(exc),
        ...
        return {
            "action_id": action_id,
            "status": state.get("recovery_status", "failed"),
            "error": str(exc),
            "failure_class": failure_class,
        }
    finally:
